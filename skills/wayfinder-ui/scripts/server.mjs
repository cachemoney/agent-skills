#!/usr/bin/env node
// wayfinder-ui server. Plain Node, zero dependencies, no build step.
//
// Commands:
//   new           --destination D [--notes N] [--doc P] [--tracker T]
...
//   visual.html   — optional prototype or roadmap diagram
...
const die = (msg, code = 2) => {
  process.stderr.write(`wayfinder: ${msg}\n`);
  process.exit(code);
};
...
function cmdNew(o) {
  ...
  const destination = typeof o.destination === "string" ? o.destination : typeof o.topic === "string" ? o.topic : "";
  const notes = typeof o.notes === "string" ? o.notes : "";
  const doc = typeof o.doc === "string" ? o.doc : `docs/${destination ? destination.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") : "epic"}-roadmap.md`;
  const trackerType = typeof o.tracker === "string" ? o.tracker : "local";
  ...
  print({ session, key, project, id: path.basename(session), destination, doc });
}
...
function cmdServe(o) {
  ...
  const srv = http.createServer(async (req, res) => {
    ...
    if (req.method === "GET" && pathname === "/") {
      if (!fs.existsSync(page)) return send(res, 200, "<!doctype html><title>wayfinder</title><p>page.html not found</p>", "text/html; charset=utf-8");
...
function applyPatch(state, p, now) {
  ...
  for (const [k, v] of fieldsOf(stampTimes(p, state, now), "the patch")) {
    ...
    else if (k === "tracker") out.tracker = mergeOne(out.tracker, v, "tracker");
...
function validateState(s) {
  ...
  for (const k of ["destination", "notes", "doc", "project", "created"]) {
    if (k in s) need(str(s[k]), `${k} must be a string`);
  }
  if ("agent" in s) {
    need(isObj(s.agent), "agent must be an object");
    if ("status" in s.agent) need(s.agent.status === "waiting" || s.agent.status === "working", 'agent.status must be "waiting" or "working"');
