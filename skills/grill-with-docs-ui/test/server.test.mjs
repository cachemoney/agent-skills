// Tests for grill-with-docs-ui server.mjs: session creation, serve, wait, url, sessions, pending, patch.
...
const home = mkdtempSync(join(tmpdir(), "grill-docs-home-"));
...
test("new: outside git the key comes from the cwd; state.json skeleton is written", () => {
  ...
  assert.equal(state.topic, "Core Domain");
...
test("new: a git repo and one of its worktrees share one key; two sessions never collide", () => {
  const repo = tmp("gdu-repo-");
  const git = (args, cwd) =>
    execFileSync("git", ["-c", "user.email=t@t", "-c", "user.name=t", ...args], { cwd, stdio: "pipe" });
  git(["init", "-q", "-b", "main"], repo);
  git(["commit", "-q", "--allow-empty", "-m", "init"], repo);
  const wt = join(tmp("gdu-wt-"), "wt");
  git(["worktree", "add", "-q", wt, "-b", "side"], repo);

  const a = newSession(repo, "Repo Topic"),
    b = newSession(wt, "Worktree Topic"),
    c = newSession(repo, "Another Topic");
...
test("serve: ready line + server.json, page, state, send appends same line it prints, bad bodies are 400, seq survives restart", async (t) => {
  const { session } = newSession(tmp("gdu-s-"));
  ...
  assert.equal(state.topic, "Architecture Redesign");
...
test("serve: /send rejects mismatched Origin, allows same-origin and no-Origin requests", async (t) => {
  const { session } = newSession(tmp("gdu-o-"));
  const s = await startServe(session);
  t.after(s.stop);
  const actions = [{ type: "claim_ticket", ticket_id: "t1" }];
...
test("serve: /visual serves session visual.html, 404 JSON when absent", async (t) => {
  const { session } = newSession(tmp("gdu-v-"));
  const s = await startServe(session);
  t.after(s.stop);
  ...
  const html = "<!doctype html><title>Visual</title><h1>Domain Architecture</h1>";
...
test("serve: reuses last port from server.json, falls back to ephemeral when taken; --port wins", async (t) => {
  const { session } = newSession(tmp("gdu-p-"));
...
test("url: prints running server url; fails fast when there is none", async (t) => {
  const { session } = newSession(tmp("gdu-u-"));
...
test("pending: prints events past agent.handled, nothing when caught up", async (t) => {
  const { session } = newSession(tmp("gdu-pd-"));
  const s = await startServe(session);
  t.after(s.stop);
...
test("patch: merges tickets and auto-calculates frontier/blocked statuses", () => {
  const { session } = newSession(tmp("gdu-patch-"));
  const patch1 = {
    agent: { status: "waiting", handled: 1 },
    tickets: [
      { id: "t1", title: "Storage Engine Selection", type: "research", question: "Which DB?" },
      { id: "t2", title: "Consensus Model", type: "grilling", durable: true, blocked_by: ["t1"], question: "Raft vs Paxos?" },
    ],
  };
  ...
  // Resolve t1 -> t2 promoted to frontier
  const patch2 = {
    tickets: [
      { id: "t1", status: "resolved", answer: { summary: "Adopt RocksDB" } },
    ],
  };
  ...
  assert.equal(t2_promoted.durable, true);
});

test("patch: detects cycles in ticket dependencies", () => {
  const { session } = newSession(tmp("gdu-cycle-"));
  const patch = {
    tickets: [
...
test("patch: auto-stamps timestamps on missing dates", () => {
  const { session } = newSession(tmp("gdu-stamp-"));
  const patch = {
...
test("patch: null deletes keys", () => {
  const { session } = newSession(tmp("gdu-del-"));
  const patch1 = {
    active_ticket_id: "t1",
...
test("patch: merges fog, terms, and adrs properly", () => {
  const { session } = newSession(tmp("gdu-fog-terms-"));
  const patch = {
    fog: [
    ...
    adrs: [
      "docs/adr/0001-partition-strategy.md",
    ],
  };
  run(["patch", "--session", session], { input: JSON.stringify(patch) });
  ...
  assert.equal(state.adrs.length, 1);
  assert.equal(state.adrs[0], "docs/adr/0001-partition-strategy.md");
});

test("wait: blocks until new send newer than --after, timeout exits 3", async (t) => {
  const { session } = newSession(tmp("gdu-wait-"));
  const s = await startServe(session);
  t.after(s.stop);
